function [lambda, fval] = ExampleFitDwellTimes_mstate(X, Delta,m,scal)
...
data = X'; 
for mFluor = 1:s(1) 
    frameStates = data(1:s(2)-ftime(mFluor),mFluor);
...
if m
	[rates, fval] = exp_fit_preds(offTimeMes',initial_rate/scal,m); %exponential fitting for multiple off states. 
end 
s1 = 1/pd_on.mu; 
